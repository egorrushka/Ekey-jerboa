...
public:
  ...
  void SetTargetPublicKey(const uint64_t* pubKeyX, int pubKeyParity);

  // OPT: Загрузка целевого hash160 в constant memory GPU.
  // Вызывается из Vanity.cpp когда numAddresses == 1.
  // Активирует SINGLE_TARGET_MODE — быстрый путь без global memory reads.
  void SetHash160Target(const uint8_t* hash160, bool isSingle);
